...
run_evd <- function(countries) {
  S       <- which(rownames(mf) %in% as.character(which(ruggedness[,3] %in% countries)))
  res     <- estimate_dfb_evd(y, X1, Xother, S, block_count=block_count)
  p       <- res$params
  loc_adj <- p[1] + p[2] * log(block_count)
  Sdfb    <- abs(res$set_dfb)
  data.frame(`|DFBETA|`          = round(Sdfb, 4),
             `Adjusted Location` = round(loc_adj, 4),
             `EVD Scale`         = round(p[2], 4),
             `P-value`           = round(1 - pgumbel(Sdfb, loc_adj, p[2]), 4),
             check.names=FALSE)
}

summary_df <- cbind(Countries=names(country_sets),
