...
clean_data <- \(df) df[, colSums(is.na(df)) == 0] |>
  select(-communityname, -state, -fold)

fit_mdl <- \(df) lm(ViolentCrimesPerPop ~ ., data=clean_data(df))

mdl    <- fit_mdl(data)
mdl_rm <- fit_mdl(data[-c(463, 1583), ])
summary(mdl); summary(mdl_rm)
...
evd_plot <- function(y, X1, Xother, S, block_count, title) {
  ...
  cat(title, "p-value:", 1 - pgumbel(Sdfb, loc_adj, p[2]), "\n")
}

# ── Analyses ──────────────────────────────────────────────────────────────────
par(mfrow=c(1, 3))
S1 <- c(463, 1583); S2 <- c(1271, 669)
...
evd_plot(y[-S1], X1[-S1], Xother[-S1,],
         S=c(1271-sum(S1<1271), 669-sum(S1<669)),   # adjust indices
         block_count=40, "Second Set")

# ── Coefficient Comparison ────────────────────────────────────────────────────
mdl_rm2  <- fit_mdl(data[-c(S1, S2), ])
coefs <- sapply(list(mdl, mdl_rm, mdl_rm2),
                \(m) summary(m)$coefficients["racePctBlack", 1]) |>
  setNames(c("orig","part1","all"))

cat(sprintf("Shifts:  part1-orig = %.4f | all-orig = %.4f | all-part1 = %.4f\n",
            coefs["part1"]-coefs["orig"],
            coefs["all"]-coefs["orig"],
            coefs["all"]-coefs["part1"]))
cat(sprintf("Ratios:  all/orig = %.4f | part1/orig = %.4f | part1/all = %.4f\n",
            coefs["all"]/coefs["orig"],
            coefs["part1"]/coefs["orig"],
